...
f_out_write = open("robExt-6F4447AE375C04676193B7AE4A957FA2-out.tex", "w")
...
for program_label, program_stats in stats.items():
  if program_label in program_label_mapping:
    ...
    offset = width * multiplier
    ax.bar(x + offset, vals_index_query, width, color="#ED0020", label=f"{program_label_mapping[program_label]}: Index query")
    multiplier += 1
    offset = width * multiplier
    ax.bar(x + offset, vals_index_target, width, color="#107AB0", label=f"{program_label_mapping[program_label]}: Index target")
    multiplier += 1
    offset = width * multiplier
    ax.bar(x + offset, vals_read_lis, width, color="#FFDE21", label=f"{program_label_mapping[program_label]}: Read LIS")
    multiplier += 1
    offset = width * multiplier
    ax.bar(x + offset, vals_read_lis_cutoff, width, color="#006A4E", label=f"{program_label_mapping[program_label]}: Find best LIS region")
    multiplier += 1
    offset = width * multiplier
    ax.bar(x + offset, vals_read_align, width, color="#33006F", label=f"{program_label_mapping[program_label]}: Alignments")
    multiplier += 1
    offset = width * multiplier
