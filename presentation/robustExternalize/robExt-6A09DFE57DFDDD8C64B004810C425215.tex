...
f_out_write = open("robExt-6A09DFE57DFDDD8C64B004810C425215-out.tex", "w")
...
for program_label, program_stats in stats.items():
  if program_label in program_label_mapping:
    ...
    rects = ax.bar(x + offset, vals, width, hatch=hatch_per_type["start"])
    ax.bar(x + offset, [program_stats[case]["read_bwt_pad_end"]["t_total"] for case in cases], width, label=program_label_mapping[program_label], bottom=vals, hatch=hatch_per_type["end"])
