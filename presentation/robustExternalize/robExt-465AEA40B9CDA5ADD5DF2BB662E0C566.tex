...
f_out_write = open("robExt-465AEA40B9CDA5ADD5DF2BB662E0C566-out.tex", "w")
...
for program_label, program_stats in stats.items():
  if program_label in program_label_mapping:
    ...
    vals_index_query = [(program_stats[case]["ref_chunk_building"]["t_total"] if "ref_chunk_building" in program_stats[case] else 0)/1000 for case in cases]
    vals_index_target = [(program_stats[case]["read_index_building"]["t_total"] if "read_index_building" in program_stats[case] else 0)/1000 for case in cases]
    ...
    for (series, color, name) in [
      (vals_index_query, "#ED0020", "Index query"),
      (vals_index_target, "#107AB0", "Index target"),
