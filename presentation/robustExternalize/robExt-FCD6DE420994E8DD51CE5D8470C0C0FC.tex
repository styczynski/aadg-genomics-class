...
f_out_write = open("robExt-FCD6DE420994E8DD51CE5D8470C0C0FC-out.tex", "w")
...
for program_label, program_stats in stats.items():
  if program_label in program_label_mapping:
    ...
    multiplier += 1
    offset = width * multiplier
    ax.bar(x + offset, vals_read_lis, width, color="#FFDE21", label=f"{program_label_mapping[program_label]}: Index target")
    multiplier += 1
    offset = width * multiplier
    ax.bar(x + offset, vals_read_lis_cutoff, width, color="#006A4E", label=f"{program_label_mapping[program_label]}: Index target")
    multiplier += 1
    offset = width * multiplier
    ax.bar(x + offset, vals_read_align, width, color="#33006F", label=f"{program_label_mapping[program_label]}: Index target")
    multiplier += 1
    offset = width * multiplier
