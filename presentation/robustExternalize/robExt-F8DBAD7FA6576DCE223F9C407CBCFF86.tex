...
f_out_write = open("robExt-F8DBAD7FA6576DCE223F9C407CBCFF86-out.tex", "w")
...
for program_label, program_stats in stats.items():
  if program_label in program_label_mapping:
    ...
    rects = ax.bar(x + offset, vals, width, hatch="0", color=color_per_solution[program_label])
    ax.bar(x + offset, [program_stats[case]["read_bwt_pad_end"]["t_total"] for case in cases], width, label=program_label_mapping[program_label], bottom=vals, hatch="//", color="white", edgecolor=color_per_solution[program_label])
