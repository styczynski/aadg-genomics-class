...
f_out_write = open("robExt-B2830199579E8C57DFC139F222E33D47-out.tex", "w")
...
for program_label, program_stats in stats.items():
  if program_label in program_label_mapping:
    ...
    rects = ax.bar(x + offset, [program_stats[case]["read_bwt_pad_begin"]["t_total"] for case in cases], width, label="st-"+program_label_mapping[program_label])
    ax.bar_label(rects, padding=3)
    multiplier += 1
    offset = width * multiplier
    rects = ax.bar(x + offset, [program_stats[case]["read_bwt_pad_end"]["t_total"] for case in cases], width, label="en-"+program_label_mapping[program_label])
    ax.bar_label(rects, padding=3)
    multiplier += 1
...
ax.set_title("Number of padding adjusted via BWT")
