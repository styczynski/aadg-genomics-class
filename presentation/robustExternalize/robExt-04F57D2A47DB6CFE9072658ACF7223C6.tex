...
f_out_write = open("robExt-04F57D2A47DB6CFE9072658ACF7223C6-out.tex", "w")
...
for program_label, program_stats in stats.items():
  if program_label in program_label_mapping:
    ...
    rects = ax.bar(x + offset, vals, width, hatch=hatch_per_type["start"], color=color_per_solution[program_label])
    ax.bar(x + offset, [program_stats[case]["read_bwt_pad_end"]["t_total"] for case in cases], width, label=program_label_mapping[program_label], bottom=vals, hatch=hatch_per_type["end"], color=color_per_solution[program_label])
