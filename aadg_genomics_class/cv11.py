...
def measure_class_distance(truth_class, test_path, test_sig, classes, training_classes):
    ...
    for cls in classes:
        ...
        i = 0
        chunk_size = 1000000
        for start_index in range(0, len(combine), chunk_size):
            #for v in combine.tolist():
                # ((tupx & kmermask) >> occmask_len, (tupx & occmask))
            gc_collect()
            for v in combine[start_index:start_index+chunk_size].tolist():
                is_test = (v & 1)
                signo = (v & signo_mask) != 0
                vs = v >> 1
                kmer = (vs & mask) >> OCC_MASK_LEN
                occ = (vs & occ_mask)
                bangla += 1
                # print(i)
...
def run_classifier_pipeline(
    ...
    if not super_debug:
        ...
        for (testing_dataset_path, speed_1m, test_sig, xxx) in pool.starmap(load_data_class_mp, p_args):
            # if dump_tests:
            #     import pickle
            #     with open("testpickles/"+testing_dataset_path.split("/")[-1]+".cv11.out.pkl", 'wb') as dump_file:
            #         pickle.dump((testing_dataset_path, speed_1m, test_sig, xxx), dump_file)
