    ...
    for fasta_file_gz in datasets_paths:
        for line in chain(gzip.open(fasta_file_gz, 'rt'), [">"]):
            if line[0] != '>':
                ...
                loaded_reads += 1
            elif loaded_reads > DATASET_CHUNK_SIZE or len(line) == 1:
                ...
                if False:
                    encepence = KMER_LEN
                    #seq_buffer_b = b''.join((mmh3.mmh3_x64_128_digest(seq_arr[i:i+encepence]) for i in range(0, len(seq_arr), encepence)))
                    #kmers = np.frombuffer(seq_buffer_b, dtype=np.uint32)
                    kmers = np.array([mmh3.mmh3_x64_128_digest(seq_arr[i:i+encepence]) for i in range(0, len(seq_arr), 1)])
                else:
