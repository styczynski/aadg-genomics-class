...
MIN_LIS_EXTENSION_WINDOW_SCORE_BASE = 0.45
...
def run_match_align_bwt(q, t):
    ...
    for (
        ...
        for threshold_i in range(len(thresholds)):
            ...
            off, off_score = run_bwt(bwt, bwt_rev, query_string, threshold, suffix_array)
    ...
    return off_l, off_r, realign_right, off_score
...
def get_scores_base(matches, n, max_diff, query_len):
...
from numpy import searchsorted, iinfo, full, zeros

zzz = 0
def get_scores_window_diff(matches, n, max_diff, query_len):
    print(f"get_scores_window_diff({n})")
    global zzz
    matches = matches.tolist()
    ...
    if n < MIN_LIS_EXTENSION_WINDOW_LEN:
        ...
        parent = [maxsize]*(n+1)
        increasingSub = [maxsize]*(n+1)
        increasingSubMatches = array([maxsize]*(n+1))
        increasingSubMatchesX = array([maxsize]*(n+1))
        for i in range(n):
            start = 1
            end = longest_seq_len

            pp = searchsorted(increasingSubMatchesX, matches[i][0] - max_diff)
            p = searchsorted(increasingSubMatches[pp:], matches[i][1])

            # while start <= end:
            #     middle = (start + end) // 2
            #     if increasingSubMatches[middle] >= matches[i][1] or matches[increasingSub[start]][0] + max_diff < matches[i][0]:
            #         end = middle - 1
            #     else:
            #         start = middle + 1    

            #start = pp
            #print(f"start, end = {start}, {end} p = {p} pp = {pp} ({local_longest_seq_len}, {longest_seq_len})")
            start = p
            parent[i] = increasingSub[start-1]
            increasingSub[start] = i
            increasingSubMatches[start] = matches[i][1]
            increasingSubMatchesX[start] = matches[i][0]
            if start > longest_seq_len:
                longest_seq_len = start
        ...
        if __debug__:
            track_counter('lis_length', int(longest_seq_len))
        ...
        for start in range(0, longest_seq_len):
            ...
            if wnd_len > MIN_LIS_EXTENSION_WINDOW_LEN:
                ...
                score = (min(estimated_matches_q, estimated_matches_t) - max(spaces, spaces_q))/query_len
        ...
        #print(sorted(scores))
        r =  sorted(scores.values(), reverse=True)[:MAX_ACCEPTED_LIS_EXTENSION_WINDOWS_COUNT]
        #print(r)
        return r
...
from numpy import maximum, diff, cumsum, roll, minimum


def get_scores_window_diff0npfullsickifs(matches, n, max_diff, query_len):
    global zzz
   # matches = matches.tolist()

    if n < MIN_LIS_EXTENSION_WINDOW_LEN:
        if __debug__:
            track_block('read_lis_fullsick', end=True)
        pass
    else:
        longest_seq_len = 0
        parent = [maxsize]*(n+1)
        ...
        current_node = increasingSub[longest_seq_len]
        lll = [current_node]*longest_seq_len
        ...
        for j in range(longest_seq_len-1, 0, -1):
            current_node = parent[current_node]
            lll[j-1] = current_node
            # lis_t[j-1] = matches[current_node, 0]
            # lis_q[j-1] = matches[current_node, 1]
        lis = take(matches, lll, axis=0)

        dcs = cumsum(maximum(diff(maximum(lis[:, 0], lis[:, 1])), KMER_LEN) - KMER_LEN)
        sss = searchsorted(lis[:,0], lis[:,0]+max_diff-lis[:,1])-1
        #sq = cumsum(maximum(diff(lis_q), KMER_LEN) - KMER_LEN).tolist()
        
        if __debug__:
            track_block('read_lis_fullsick', end=True)
            track_block('read_lis_cutoff_fullsick')

        if __debug__:
            track_counter('lis_length_fullsick', longest_seq_len)
        ...
        start = 0
        end = sss[0] #bisect_right(lis_t, lis_t[0] + max_diff - lis_q[0]) - 1

        #wynik[i]*query_len = min(lis[sss[i], 1]-lis[i,1], lis[sss[i], 0]-lis[i, 0]) - dcs[sss[i]-1] + dcs[i-1]

        dcs_shift = roll(concatenate((dcs, [0])), 1)
        #dcs_wnd = dcs_shift - dcs[sss-1]
        wnd_len = sss - arange(1,len(sss)+1,1)

        # dbg1 = minimum(lis[sss,1]-lis[:,1], lis[sss,0]-lis[:,0])
        # dbg2 = dcs_shift
        # dbg3 = dcs[sss-1]
        # print(f"dbg1(len)={len(dbg1)} dbg2(len)={len(dbg2)} dbg3(len)={len(dbg3)}")
        wyniki = minimum(lis[sss,1]-lis[:,1], lis[sss,0]-lis[:,0]) - dcs_shift + dcs[sss-1]

        for start in range(1, longest_seq_len):
            if wnd_len[start] > MIN_LIS_EXTENSION_WINDOW_LEN:
                score = wyniki[start-1]/query_len
                score_1 = score_2
                score_2 = score_3
                score_3 = score
                ...
                if score_2 > score_1 and score_2 > score_3:
                    # Local maximum
                    if score_2 > MIN_LIS_EXTENSION_WINDOW_SCORE:
                        score_bucket = lis[end, 0] // score_bucket_div
                        if (score_bucket not in scores) or (score_bucket in scores and score_2 > scores[score_bucket][0]):
                            scores[score_bucket] = (score_2, lis[start, 0], lis[end, 0], lis[start, 1], lis[end, 1])
                        #scores.append((score_2, lis_t[start], lis_t[end], lis_q[start], lis_q[end]))
                    #print(f"score={match_score} start={match_start_t}")
                if end >= longest_seq_len - 1:
                    break
        if score_3 > score_2 and score_3 > score_1:
            if score_3 > MIN_LIS_EXTENSION_WINDOW_SCORE:
                score_bucket = lis[end, 0] // score_bucket_div
                if (score_bucket not in scores) or (score_bucket in scores and score_3 > scores[score_bucket][0]):
                     scores[score_bucket] = (score_3, lis[start, 0], lis[end, 0], lis[start, 1], lis[end, 1])
            
        #scores = scores.values()
        #print(sorted(scores))
        return sorted(scores.values(), reverse=True)[:MAX_ACCEPTED_LIS_EXTENSION_WINDOWS_COUNT]
        #print(scores)
        # print(scores)
        # zzz += 1
        # if zzz > 100:
...
# r_[True, a[1:] > a[:-1]] & r_[a[:-1] > a[1:], True]
from numpy import greater, r_, c_, lexsort, argwhere, where

def get_scores_window_diff0npfullsickbs_minmax_ultra(matches, n, max_diff, query_len):
    global zzz
   # matches = matches.tolist()
    ...
    if n < MIN_LIS_EXTENSION_WINDOW_LEN:
        ...
        # parent = [maxsize]*(n+1)
        # increasingSub = [maxsize]*(n+1)
        # increasingSubX = array([matches[0, :]]*(n+1))
        # for i in range(n):
        #     z = argwhere(increasingSubX[:longest_seq_len,0]>=matches[i, 0] - max_diff).flatten() # OK pozycje
        #     if len(z) == 0:
        #         start = longest_seq_len+1
        #     else:
        #         p = searchsorted(increasingSubX[z, 1], matches[i, 1], side='left')
        #         if len(z) == p and p > 0:
        #             start = z[p-1]+1
        #         else:
        #             start = z[p]-1
        #     parent[i] = increasingSub[start-1]
        #     increasingSub[start] = i
        #     increasingSubX[start] = matches[i]
        #     if start > longest_seq_len:
        #         longest_seq_len = start
        ...
        #     current_node = parent[current_node]
        #     lll[j-1] = current_node
        lis = matches #take(matches, lll, axis=0)
        longest_seq_len = len(lis)
        ...
        wyniki = minimum(lis[sss,1]-lis[:,1], lis[sss,0]-lis[:,0]) - ((dcs_shift + dcs[sss-1])/2)
        #candidate_map = greater(wnd_len, MIN_LIS_EXTENSION_WINDOW_LEN) & greater(wyniki, MIN_LIS_EXTENSION_WINDOW_SCORE*query_len) & r_[True, wyniki[1:] > wyniki[:-1]] & r_[wyniki[:-1] > wyniki[1:], True]
        candidate_map = greater(wnd_len, MIN_LIS_EXTENSION_WINDOW_LEN) & r_[True, wyniki[1:] > wyniki[:-1]] & r_[wyniki[:-1] > wyniki[1:], True]
        trytytka = c_[lis[sss,0][candidate_map]//score_bucket_div, wyniki[candidate_map], lis[sss][candidate_map], lis[candidate_map]]
        sind = lexsort((trytytka[:,0], -trytytka[:,1]))
        scores = dict([(key, (s, st, et, sq, eq)) for (key, s, et, eq, st, sq) in trytytka[sind].tolist()])
        return sorted(scores.values(), reverse=True)[:MAX_ACCEPTED_LIS_EXTENSION_WINDOWS_COUNT]


def get_scores_window_diff0npfullsickbs_minmax(matches, n, max_diff, query_len):
    global zzz
   # matches = matches.tolist()
    ...
    if n < MIN_LIS_EXTENSION_WINDOW_LEN:
        ...
        for i in range(n):
            ...
            while start <= end:
                if start != 1 and matchesl[increasingSub[start]][0] + max_diff < matchesl[i][0]:
                    break
                middle = (start + end) // 2
                if matchesl[increasingSub[middle]][1] >= matchesl[i][1]:
                    end = middle - 1
                else:
                    start = middle + 1    
            parent[i] = increasingSub[start-1]
        ...
        trytytka = c_[lis[sss,0][candidate_map]//score_bucket_div, wyniki[candidate_map], lis[sss][candidate_map], lis[candidate_map]]
        sind = lexsort((trytytka[:,0], -trytytka[:,1]))
        scores = dict([(key, (s, int(st), int(et), int(sq), int(eq))) for (key, s, et, eq, st, sq) in trytytka[sind].tolist()])
        return sorted(scores.values(), reverse=True)[:MAX_ACCEPTED_LIS_EXTENSION_WINDOWS_COUNT]



def get_scores_window_diff0npfullsickbs(matches, n, max_diff, query_len):
    global zzz
   # matches = matches.tolist()
...
def get_scores_window_diff0npfullsick(matches, n, max_diff, query_len):
    global zzz
   # matches = matches.tolist()
...
def get_scores_window_diff0npfull(matches, n, max_diff, query_len):
    global zzz
   # matches = matches.tolist()
    ...
    if n < MIN_LIS_EXTENSION_WINDOW_LEN:
        ...
        for start in range(0, longest_seq_len):
            ...
            new_end = sss[start] #bisect_right(lis_t, lis_t[start] + max_diff - lis_q[start], lo=end) - 1
            # for i in range(end+1, new_end+1):
            #     #print(f"add {i-1}")
            #     spaces += max(lis_t[i] - lis_t[i-1], KMER_LEN) - KMER_LEN
            #     spaces_q += max(lis_q[i] - lis_q[i-1], KMER_LEN) - KMER_LEN
            end = new_end
            # spaces = st[end-1] - (st[start-1] if start != 0 else 0)
            # spaces_q = sq[end-1] - (sq[start-1] if start != 0 else 0)
            spaces = dcs[end-1] - (dcs[start-1] if start != 0 else 0)

            wnd_len = end - start + 1
            #print(wnd_len)
            if wnd_len > MIN_LIS_EXTENSION_WINDOW_LEN:
                estimated_matches_q = lis[end, 1] - lis[start, 1]
                estimated_matches_t = lis[end, 0] - lis[start, 0]
                score = (min(estimated_matches_q, estimated_matches_t) - spaces)/query_len
                #print(f"score={score} diff_sum={spaces}")
                score_1 = score_2
                score_2 = score_3
                score_3 = score
...
def get_scores_window_diff0nplis(matches, n, max_diff, query_len):
    global zzz
   # matches = matches.tolist()
    ...
    if n < MIN_LIS_EXTENSION_WINDOW_LEN:
        ...
        track_block('read_lis_fullsick_alg')
        matchesl = matches.tolist()
        longest_seq_len = 0
        parent = [maxsize]*(n+1)
        increasingSub = [maxsize]*(n+1)
        #increasingSubM0 = [matchesl[0][0]]*(n+1)
        #increasingSubM1 = [matchesl[0][1]]*(n+1)
        #increasingSubM = [(0, 0)]*(n+1)
        # TRY WITH CACHE ARRAY
        # for i in range(n):
        #     start = 1
        #     end = longest_seq_len
        #     #magickey = lambda x: (x[1] if (x[0] >= matchesl[i][0]-max_diff) else 0)
        #     # magickey = lambda x: (x[0] >= 55, x[1])
        #     # bisect_left(leled, (True, 100), key=magickey)
        #     #bisect_magic = bisect_left(increasingSubM, (True, matchesl[i][1]), key=magickey)
        #     while start <= end:
        #         if increasingSubM[start][0] + max_diff < matchesl[i][0]:
        #             break
        #         middle = (start + end) // 2
        #         if increasingSubM[start][1] >= matchesl[i][1]:
        #             end = middle - 1
        #         else:
        #             start = middle + 1    
        #     #print(f" bisect_magic = {bisect_magic} (compared to start={start})")
        #     parent[i] = increasingSub[start-1]
        #     increasingSub[start] = i
        #     #increasingSubM0[start] = matchesl[i][0]
        #     increasingSubM[start] = matchesl[i]
        #     #increasingSubM[start] = (matchesl[i][0], matchesl[i][1])
        #     if start > longest_seq_len:
        #         longest_seq_len = start
        # import sys
        # sys.exit(1)
        ...
        track_block('read_lis_fullsick_alg', end=True)
        track_block('read_lis_fullsick_jjj')
        current_node = increasingSub[longest_seq_len]
        jjj = [current_node]*longest_seq_len
        # lis_t = [0]*longest_seq_len
        # lis_q = [0]*longest_seq_len
        # lis_t[longest_seq_len-1] = matches[current_node, 0]
        # lis_q[longest_seq_len-1] = matches[current_node, 1]
        for j in range(longest_seq_len-1, 0, -1):
            current_node = parent[current_node]
            jjj[j-1] = current_node
            # lis_t[j-1] = matches[current_node, 0]
            # lis_q[j-1] = matches[current_node, 1]

        lis = take(matches, jjj, axis=0)
        lis_t = lis[:,0]
        lis_q = lis[:,1]
        track_block('read_lis_fullsick_jjj',end=True)
        track_block('read_lis_fullsick_cumsum')
        dcs = cumsum(maximum(diff(maximum(lis_t, lis_q)), KMER_LEN) - KMER_LEN)
        track_block('read_lis_fullsick_cumsum', end=True)
        #sq = cumsum(maximum(diff(lis_q), KMER_LEN) - KMER_LEN).tolist()
...
def get_scores_window_diff0np(matches, n, max_diff, query_len):
    global zzz
   # matches = matches.tolist()
...
def get_scores_window_diff0(matches, n, max_diff, query_len, reported):
    global zzz
    matches = matches.tolist()

    if n < MIN_LIS_EXTENSION_WINDOW_LEN:
        if __debug__ and reported:
            track_block('read_lis', end=True)
        pass
    else:
        longest_seq_len = 0
        ...
        #print("FOUND LIS:")
        #print([(lis_t[i], lis_q[i]) for i in range(len(lis_t))])
        ...
        #scores = scores.values()
        #print(sorted(scores))
        #print(f"scores = {list(scores.values())} max_score={mscore}")
        return sorted(scores.values(), reverse=True)[:MAX_ACCEPTED_LIS_EXTENSION_WINDOWS_COUNT]
        #print(scores)
        # print(scores)
        # zzz += 1
                ...
                if line[0] == '>' and len(query_seq) > 0:
                    ...
                    if True: #query_id == 'read_965':
                        try:
                            ...
                            n = len(matches)
                            if __debug__:
                                track_block('cross_match', end=True)
                            ...
                            #scores = get_scores_base(matches, n, max_diff, query_len)
                            if n > 10000:
                               #print(f"CASE! n = {n}")
                               # if __debug__:
                               #     track_block('read_lis_fullsick')
                               # scoresx = get_scores_window_diff0npfullsickbs_minmax(matches, n, max_diff, query_len)
                               if __debug__:
                                   #track_block('read_lis_cutoff_fullsick', end=True)
                                   track_block('read_lis')
                               scores = get_scores_window_diff0(matches, n, max_diff, query_len, True)
                               if __debug__:
                               ...
                               # sys.exit(1)
                                ...
                                #if __debug__:
                                #   track_block('read_lis')
                                scores = get_scores_window_diff0(matches, n, max_diff, query_len, False)
                                #if __debug__:
                                #   track_block('read_lis_cutoff', end=True)
                            if len(scores) > 0:
                                ...
                                if len(scores) > 1:
                                    ...
                                    for (_, cmatch_start_t, cmatch_end_t, cmatch_start_q, cmatch_end_q) in scores:
                                        ...
                                        if __debug__:
                                            track_block('lis_read_dp')
                                        _, ct_end_pad, _, cdp_score = run_match_align_bwt(
                                            query_seq,
                                        ...
                                        if __debug__:
                                            track_block('lis_read_dp', end=True)
                            ...
                            if not lis_accepted or abs(match_end_t - match_start_t) > max_diff + relative_extension:
                                ...
                                for i in range(2):
                                    ...
                                    t_begin_pad, t_end_pad, should_realign_right, _ = run_match_align_bwt(
                                        query_seq,
                                        target_seq[t_begin:t_end],
                                    )
                                    if __debug__:
