...
if __debug__:
    ...
    def compare_to_expected_results(reads_file_path, output_file_path):
        import os.path
        import csv

        reads_parts = reads_file_path.split(".")
        if len(reads_parts) == 0:
            return
        elif len(reads_parts) == 1:
            reads_parts.append("txt")
        else:
            reads_parts[-1] = "txt"
        results_file_path = ".".join(reads_parts)

        if not os.path.isfile(results_file_path):
            print(f"Cannot read {results_file_path} so no output statistics will be generated.")
            print("You see this information, because you're in the debug mode.")
            return
        ...
        for (seq_id, (expected_s, expected_e)) in expected_coords.items():
            ...
            if seq_id not in actual_coords:
                ...
                track_counter(f'qc_results_max_diff', max_diff)
                track_counter(f'qc_results_sum_diff', sum_diff)
                track_counter(f'qc_results_start_diff', abs(diff_start))
                track_counter(f'qc_results_end_diff', abs(diff_end))
        ...
        track_counter(f'qc_results_count_bad', bad_count)
        track_counter(f'qc_results_count_ok', ok_count)
        track_counter(f'qc_results_count_unmapped', unmapped_count)

        track_counter(f'qc_results_count_bad_percent', bad_count / len(expected_coords) * 100)
        track_counter(f'qc_results_count_ok_percent', ok_count / len(expected_coords) * 100)
        track_counter(f'qc_results_count_unmapped_percent', unmapped_count / len(expected_coords) * 100)

        if ok_count > 0:
            for (key, count) in qual_count.items():
                track_counter(f'qc_results_count_qual_{key.lower()}', count)
                track_counter(f'qc_results_count_qual_{key.lower()}_percent', count/ok_count*100)
...
def run_aligner_pipeline(
                        ...
                        try:
                            ...
                            if n < MIN_LIS_EXTENSION_WINDOW_LEN:
                                ...
                                if len(scores) > 1:
                                    ...
                                    if __debug__:
                                        track_counter('lis_no_results_fallback_run')
                                    # problem!
                                    longest_seq_len = 0
                                    parent = [maxsize]*(n+1)
                                    increasingSub = [maxsize]*(n+1)
                                    ...
                                    if match_score > -max_diff:
                                        ...
                                        if __debug__:
                                            track_counter('lis_no_results_fallback_failure')
                                else:
    ...
    if __debug__:
        compare_to_expected_results(reads_file_path, output_file_path)
        print_track_info()
        import os
        os._exit(0) # Faster exit in debug mode 
