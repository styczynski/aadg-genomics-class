...
def inexact_recursion(s, i, diff, k, l, prev_type):
    ...
    for char in ALPHABET:
        temp_k = _global_bwt_c[char] + get_O(char, k-1) + 1
        temp_l = _global_bwt_c[char] + get_O(char, l)
        ...
        if temp_k <= temp_l:
            ...
            if char == s[i]:
                ...
                # Mismatch
                sub_result, has_result = inexact_recursion(s, i-1, diff-COST_MISMATCH, temp_k, temp_l, 3)
                if has_result:
                    return sub_result, True
    return set(), False
...
def inexact_search(bw, bwr, s, diff):
    ...
    _global_bwt_c = compute_C(tot)
    _global_bwt_d = compute_D(s, _global_bwt_c, Oprime, bw)
